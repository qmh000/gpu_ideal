 ...
 *      Author: teng
 */
#include "../include/query_context.cuh"
...
void query_context::print_stats(){
	...
	if(collect_latency){
		for(auto it:vertex_number){
			std::cout<<it.first<<"\t"<<it.second<<"\t"<<latency[it.first]/it.second<<std::endl;
		}
	}
...
query_context get_parameters(int argc, char **argv){
	...
	desc.add_options()
		...
		("source,s", po::value<std::string>(&global_ctx.source_path), "path to the source")
		("target,t", po::value<std::string>(&global_ctx.target_path), "path to the target")
